...
var attNames = ["VCR", "PCR", "FEMA", "TEMP", "PRECIP"];
...
function setMap(){
  ...
  var map = new L.map('map').setView([50, -101], 3)
...
d3.json("data/states_data2.json", function(error, jsonData) {
  ...
  jsonData.features.forEach(function(d){
    ...
    d.properties[key] = +d.properties[key];
  ...
  //visualize pcp
    ...
    .data(pcpdata)
    .color(function(d) {
      if (d[expressed]) { console.log(d[expressed]); return recolorMap(d[expressed]); }
      else { return "#ccc"; }; })
      .render().brushable()
  ...
  function highlight(data){
    ...
    var labelAttribute = "<h1>"+ roundRight(props[expressed]) + "</h1><br><b>Units " + expressed + "</b>:" + props[key]+ ""; //label content
